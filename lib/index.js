...
class Coupon {
   ...
   */
  static async tokensOfCustomer () {
    const arr = await NFT.tokensOfOwner()
    const arr2 = await map(arr, async elem => NFT.tokenUri(elem))

    const arr3 = await map(arr2, async elem => ipfs.cat(elem))
